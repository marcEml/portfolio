...
import useClickOutside from '@/hooks/use-clicl-outside';
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className='fixed inset-0 z-40 h-full w-full bg-brand-navy/25 backdrop-blur-sm'
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
          <div className={cn('fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8', className)}>
            {children}
          </div>
        </>
      )}
